...
I would double-check and test and ask to rewrite some pieces. 
I would iterate in this way until the code is doing what I wish, to then move on to writing new chunks of code.
I also used the AI assistant to help debugging issues, asking questions like: can you explain me why I have this..., or 
to know what are the common practice in the community when it comes to specific choice of params for specific architectures.
For this project I did not use the AI assistant to discuss how meaningful/valid were the ideas behind the experiments or how interesting 
they could be for the scientific community. In other porjects, in the past, I did this.

In this project almost all the code has been written in the way described above.
Here down you can see a very detailed description of the use of the AI assistant for this project.
...
I was giving to the assistant the header and first rows of the sequence of attempts and the exercises created in the notebook (create\_per\_student\_sequence.ipynb).
I was describing the task to the assistant in this way:

\medskip\noindent\textbf{Build a PyTorch Dataset (raw\_data\_experiments)}
\begin{itshape}
...
\medskip\noindent\textbf{Build a PyTorch Dataset (embedded\_data\_experiments)}
...
Going back to the experiments in the raw\_data\_experiments folder, after I had a good working dataset class, I asked the AI assistant to build a model.

\medskip\noindent\textbf{Build a Model}
...
so the code was not setting required\_grad = False for the params of the VLM. So I asked the assistant to fix this. After few iterations I obtained the class
KTProbe in raw\_data\_expriments/models.py

After that I gave the AI assistant the KTProbe class and asked to modify the code so that I could fine-tune the entire VLM with LoRA.
I obtained in this way the class KTLoRA.

\medskip\noindent\textbf{Modify the Model to Use LoRA}
\textit{This is the code I am using for training a last layer on top of a VLM.
I would like to test another architecture where the entire VLM is fine-tuned using LoRA.}

After I obtained the code for Lora, I was spending some time chatting with the assistant to better understand the use of the params r
and lora\_alpha, I was tyring to better understrand where in the computation such params are used and what are the typical values
the community uses for them.

Finally I asked the AI assistant to write a standart training script using only a training and validation loop, without saving the model params.
...
For the architecture in the second experiment (embedded\_data\_experiments) I was asking the AI assistant:

\medskip\noindent\textbf{Write a smile transformer class}
\textit{Write a standard transformer architecture. The input is a padded sequence of per-student history 
embeddings of shape BxTxD, interleaved exercise and interaction embedding vectors of dimension D, together with a BxT boolean attention 
mask marking real tokens versus padding. The output is a B tensor of logits giving, per student, the predicted probability of solving the next, target, exercise.}
